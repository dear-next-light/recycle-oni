...
if(state==='title'){
ov.innerHTML=`<div class="panel"><div class="eyebrow">RECYCLE ONI / v0.1.1</div><h1>リサイクルの鬼</h1><p>${bi('「まだ使える。」','Still useful.')}${bi('小鬼と施設を配置して、8回の回収日を乗り切れ。','Arrange your Oni and facilities. Survive 8 collection days.')}</p><div class="steps"><span>${bi('01 救出','Rescue')}</span><span>${bi('02 運んで処理','Carry & recycle')}</span><span>${bi('03 現場を強化','Upgrade')}</span></div><p>${bi('縦でも横でも。ピンチで拡大、ドラッグで移動。','Portrait or landscape. Pinch to zoom, drag to pan.')}</p><div id="start-slot"></div><p>BEST ¥${saved.profit.toLocaleString()} · ${bi(`回避 ${Math.round(saved.avoid*100)}%`,'Diversion')} · FLOW ${saved.flow} · ${saved.rank}</p></div>`;
...
for(const [type,f]of Object.entries(F))build.append(btn(`${f.name} ¥${f.price}`,FE[type],()=>setMode({type}),'',game.money<f.price||game.facilities.length>=C.maxFacilities));
...
if(state==='wave'){
ctl.append(btn('👹 まだ使える！','Rally! · 6 seconds',()=>{if(game.command())refresh()},'primary',game.shoutUsed));const h=document.createElement('span');h.className='hint';h.innerHTML=bi('号令は1Waveに1回。配置変更はWave間。','Rally once per Wave. Reassign between Waves.');ctl.append(h);say('まだ使える。')}
...
d.innerHTML=`<h3>${bi(`${F[f.type].name} #${f.id} · Lv.${f.level}`,FE[f.type])}</h3><p>${bi(`入力 ${f.input.length}/${game.capacity(f)} ／ 処理中 ${f.jobs.length} ／ 待ち ${f.output.length}`,'Input / Processing / Output queue')}</p><p>${bi(game.synergy(f)>1?'隣接連携：処理 +10%':'近い施設ほど運搬が速い',game.synergy(f)>1?'Nearby synergy: processing +10%':'Shorter routes mean faster deliveries.')}</p>`;
...
d.innerHTML=`<h3>${bi(`小鬼 #${o.id}`,`Oni #${o.id}`)}</h3><p>${bi(`得意：${F[o.type].name}（作業 +25%）`,`Specialty: ${FE[o.type]} · Work +25%`)}</p><p>${bi(`${traits[o.trait].name}：${traits[o.trait].desc}`,`${TE[o.trait][0]}: ${TE[o.trait][1]}`)}</p><p>${bi(`体力 ${Math.round(o.stamina)}/${o.max}`,`Stamina ${Math.round(o.stamina)}/${o.max}`)}</p><p>${bi(`担当：${F[f.type].name} #${f.id}`,`Assigned: ${FE[f.type]} #${f.id}`)}</p>`;
...
const hits=game.onis.filter(o=>Math.hypot(v.x+(o.x+.5)*v.scale-x,v.y+(o.y+.5)*v.scale-y)<=Math.max(22,v.scale*.42));
...
function frame(now){const dt=Math.min(.05,(now-last)/1000||0);last=now;if(!document.hidden)game.tick(dt);sound.tick(dt,game.state==='wave');while(game.events.length){const e=game.events.shift();sound.effect(e.kind);if(['shout','flow','reuse','burn'].includes(e.kind))say(e.kind==='burn'?'……。':e.text)}ui();renderer.draw(game,selected,mode);hudTimer+=dt;
if(hudTimer>.2){hudTimer=0;$('#hud').innerHTML=`<div>${bi('波','WAVE')}<b>${Math.max(1,game.wave)}/8</b></div><div>${bi('資金','Funds')}<b>¥${game.money.toLocaleString()}</b></div><div>${bi('地球の熱','Planet heat')}<b>${Math.floor(game.heat)}</b><meter aria-label="地球の熱 / Planet heat" min="0" max="100" low="25" high="75" optimum="0" value="${game.heat}"></meter></div><div>${bi('連続処理','ONI FLOW')}<b>×${Math.floor(game.flow)}</b></div><div class="weather">${game.state==='wave'?bi(`${weatherEffects[game.weather].name} · ${Math.max(0,Math.ceil(C.duration-game.time))}秒`,`${weatherEnglish[game.weather]} · seconds`):bi('準備で最適化','Plan your routes')}</div>`;updateZoom()}requestAnimationFrame(frame)}
