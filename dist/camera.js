...
export class Camera{
 ...
 get base(){return Math.min(this.width/24,this.height/14)}
 get view(){const scale=this.base*this.zoom;return{scale,x:(this.width-24*scale)/2+this.panX,y:(this.height-14*scale)/2+this.panY}}
 clamp(){const s=this.base*this.zoom;this.panX=Math.max(-Math.max(0,(24*s-this.width)/2),Math.min(Math.max(0,(24*s-this.width)/2),this.panX));this.panY=Math.max(-Math.max(0,(14*s-this.height)/2),Math.min(Math.max(0,(14*s-this.height)/2),this.panY))}
 world(x,y){const v=this.view;return{x:(x-v.x)/v.scale,y:(y-v.y)/v.scale}}
 zoomAt(factor,x=this.width/2,y=this.height/2){const before=this.world(x,y);this.zoom=Math.max(1,Math.min(4,this.zoom*factor));const after=this.view;this.panX+=x-(after.x+before.x*after.scale);this.panY+=y-(after.y+before.y*after.scale);this.clamp()}
 pan(dx,dy){this.panX+=dx;this.panY+=dy;this.clamp()}
 fit(){this.zoom=1;this.panX=this.panY=0}
 focus(x,y){const s=this.base*this.zoom;this.panX=(12-x)*s;this.panY=(7-y)*s;this.clamp()}
}
