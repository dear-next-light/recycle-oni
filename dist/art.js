...
export class Art {
  ...
  get(name){if(!this.images.has(name)){const im=new Image();this.images.set(name,im);im.onload=()=>{this.revision++;this.onLoad()};im.onerror=()=>{im.failed=true;this.revision++;this.onLoad()};im.src=`assets/${name}-${name==='truck-empty'?'v013':(name.startsWith('oni-')||name.startsWith('run-')||name.startsWith('foreman-')||name==='cargo')?'v014':'v012'}.webp`}const im=this.images.get(name);return im.complete&&im.naturalWidth?im:null}
}
